...
class MiniAI:
    ...
    def __init__(self):
        ...
        self._last_viewer_message_time = None  # for proactive monitoring

        # Khởi tạo messages từ lịch sử đã lưu trong memory sau khi memory đã load xong

        self.messages = self.memory.memory.get("conversation", {}).get(
            "conversation_thread", []
        )
    ...
    def _call_light_model(self, messages, temperature=0.3, max_tokens=200):
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"[Light] Ollama failed ({response.status_code}) in {duration:.1f}s, falling back"
                )
    ...
    def _call_chat_model(self, messages, temperature=0.8, max_tokens=200):
        ...
        for attempt in range(2):
            try:
                ...
                if response.status_code != 200:
                    print(
                        f"[Chat] Ollama failed ({response.status_code}) in {duration:.1f}s"
                    )
        ...
        for attempt in range(3):
            try:
                ...
                if response.status_code == 429:
                    ...
                    print(
                        f"[Groq] Rate limited (429). Waiting {wait:.1f}s before retry {attempt + 1}/3..."
                    )
                ...
                if response.status_code != 200:
                    ...
                    print(
                        f"[Chat] Groq failed ({response.status_code}) in {duration:.1f}s: {result}"
                    )
    ...
    def chat(
        ...
        if source_type == "owner":
            _illocution_type, _perlocution_hint = self.classify_illocution(
                user_input, intent
            )
        ...
        if source_type == "owner":
            _self_disclosure_hint = self._get_self_disclosure_hint(
                intent, _illocution_type
            )
        ...
        if source_type != "owner" and stream_context:
            ...
            if viewer_name:
                self.memory.add_session_item(
                    f"{viewer_name} nhắn: {user_input[:80]}", kind="session"
                )
        ...
        )  # Save dominance — restore sau viewer turn

        if source_type == "owner":
            pass  # dùng affection từ DB chính

        elif source_type == "regular_viewer":
            self.emotion.affection = (
                float(viewer_data.get("affection", 35)) if viewer_data else 35.0
            )

        elif source_type == "donor":
            self.emotion.affection = (
                min(100, float(viewer_data.get("affection", 40)) + 20)
                if viewer_data
                else 55.0
            )
        ...
        if source_type == "owner" and not reward_hint:
            ...
            elif self.conv_state.should_trigger_surprise(
                probability=0.05, min_cooldown=5
            ):
        ...
        if content:
            ...
            while parsed.get("skill_needed") and skill_depth < 2:
                ...
                if skill_content:
                    print(
                        f"[Skill] Loading skill: {skill_name} (depth {skill_depth + 1})"
                    )
        ...
        if source_type == "owner":
            ...
            self.memory.memory["conversation"]["conversation_thread"].append(
            ...
            self.memory.memory["time_tracking"]["time_gap_hours"] = (
                self.time_gap_hours or 0
            )

            self.memory.memory["relationship"]["current_affection"] = (
                self.emotion.affection
            )
    ...
    def classify_illocution(self, text: str, intent: str) -> tuple:
        ...
        assertive_signals = [
            ...
            "mình đã xong",
            "mình đã làm được",
            "mình đã giải được",  # cụ thể hơn "mình đã"
            "đã làm được",
            "đã xong",
            "đã giải được",
            "cuối cùng",
    ...
    def _get_self_disclosure_hint(self, intent: str, illocution: str) -> str:
        ...
        elif self.emotion.affection >= 65 and illocution in (
            "expressive",
            "assertive",
            "commissive",
        ):
    ...
    def extract_memory(self, text, intent, source_type="owner"):
        ...
        try:
            raw = (
                self._call_light_model(extract_prompt, temperature=0.1, max_tokens=200)
                or ""
            )
    ...
    def summarize_history(self):
        ...
        try:
            summary = (
                self._call_light_model(
                    summarize_prompt, temperature=0.4, max_tokens=120
                )
                or ""
            )
        ...
        try:
            ...
            entry_content = self._call_light_model(
                [
                    {
                        "role": "system",
                        "content": "Bạn là Lyra đang viết nhật ký. Trả về plain text.",
                    },
    ...
    def build_prompt(
        ...
        if precomputed_memory_context is not None:
            ...
            memory_context = self.memory.get_relevant_context(
                user_input, is_public=is_public
            )
        ...
        full_memory_context = "\n\n".join(
            filter(None, [memory_context, search_context, diary_hint])
        )
        ...
        lsm_hint = (
            self.conv_state.get_lsm_directive(dominance=self.emotion.dominance)
            if source_type == "owner"
            else ""
        )
        ...
        if self.is_streaming:
            ...
            situation_note += (
                "Status: You are in a PRIVATE CONVERSATION with your creator/brother.\n"
            )
        ...
        parts = [
            ...
            source_context,
            _stream_ctx,
            get_live_context_block(),  # ← injected hot state
            relationship_hint,
            mood_hint,
            user_hint,
    ...
    def compose_user_message(
        self, user_input, intent, reward_active: bool = False, ideology_idx: int = -1
    ):
    ...
    def update_stream_summary(self):
        ...
        try:
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": STREAM_ROLLING_SUMMARY_PROMPT.format(events=events_str),
                },
            ]
    ...
    def generate_stream_event_reply(self, event_type: str, context: dict = None) -> str:
        ...
        if event_type == "greeting":
            ...
            goals_str = (
                ", ".join(STREAM_GOALS) if STREAM_GOALS else "chưa có mục tiêu cụ thể"
            )
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"Stream event: {milestone_desc}. React ngắn gọn, tự nhiên.",
                },
            ]
