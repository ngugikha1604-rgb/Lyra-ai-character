    ...
    def upsert(self, item_id: str, vector: list, metadata: dict):
        ...
        try:
            requests.post(
                ...
                headers={"Api-Key": self.api_key, "Content-Type": "application/json"},
                json={
                    "vectors": [{"id": item_id, "values": vector, "metadata": metadata}]
                },
                timeout=10,
            )
        except Exception as e:
...
class MemorySystem:
    ...
    def _get_db(self):
        ...
        try:
            ...
            existing_cols = {
                row[1] for row in c.execute("PRAGMA table_info(memory_items)")
            }
    ...
    def _build_memory_dict(self, c):
        ...
        def get_self_profile(key):
            r = c.execute(
                "SELECT value FROM self_profile WHERE key=?", (key,)
            ).fetchone()
            return r[0] if r else None
    ...
    def _migrate_from_json(self, c, old):
        ...
        for item in old.get("preferences", {}).get("likes", []):
            ...
            c.execute(
                "INSERT OR IGNORE INTO memory_items (kind,value,layer,weight,saliency,source_turn) VALUES ('like',?,?,1.0,3,0)",
                (item, LAYER_USER),
            )
        for item in old.get("preferences", {}).get("dislikes", []):
            ...
            c.execute(
                "INSERT OR IGNORE INTO memory_items (kind,value,layer,weight,saliency,source_turn) VALUES ('dislike',?,?,1.0,4,0)",
                (item, LAYER_USER),
            )
        for item in old.get("facts", {}).get("goals", []):
            ...
            c.execute(
                "INSERT OR IGNORE INTO memory_items (kind,value,layer,weight,saliency,source_turn) VALUES ('goal',?,?,1.4,7,0)",
                (item, LAYER_USER),
            )
        for item in old.get("conversation", {}).get("favorite_topics", []):
            ...
            c.execute(
                "INSERT OR IGNORE INTO memory_items (kind,value,layer,weight,saliency,source_turn) VALUES ('topic',?,?,1.1,4,0)",
                (item, LAYER_USER),
            )
        for s in old.get("conversation", {}).get("chat_history_summary", []):
            ...
            if s.get("summary"):
                c.execute(
                    "INSERT OR IGNORE INTO memory_items (kind,value,layer,weight,saliency,source_turn) VALUES ('episodic',?,?,1.2,5,0)",
                    (s.get("summary", ""), LAYER_TEMPORAL),
                )
    ...
    def _detect_conflict(self, kind: str, new_value: str, c) -> "sqlite3.Row | None":
        ...
        for row in rows:
            ...
            if new_tokens and old_tokens:
                overlap = len(new_tokens & old_tokens) / max(
    ...
    def _resolve_conflict(self, kind: str, old_row, new_value: str, c, conn=None):
        ...
        if old_id:
            threading.Thread(
                target=lambda: self.pinecone.delete(f"mem_{old_id}"), daemon=True
            ).start()
        if change_note_id and self.pinecone._enabled:
            self._upsert_pinecone_async(change_note_id, "episodic", change_note, 4)

        print(
            f"[Memory] Conflict resolved: {kind} '{old_value[:40]}' → '{new_value[:40]}'"
        )
    ...
    def add_session_item(self, value: str, kind: str = "session"):
        ...
        self._session_items.append(
            {"kind": kind, "value": value, "created_at": now_iso}
        )
        ...
        try:
            ...
            with self.db_lock:
                ...
                if layer == LAYER_TEMPORAL and self.pinecone._enabled:
                    row = c.execute(
                        "SELECT id FROM memory_items WHERE kind=? AND value=?",
                        (kind, text),
                    ).fetchone()
                    if row:
    ...
    def _upsert_pinecone_async(
        self, item_id: int, kind: str, text: str, saliency: float
    ):
    ...
    def save(self):
        ...
        with self.db_lock:
            ...
            for k, v in self.memory.get("identity", {}).items():
                if v:
                    c.execute(
                        "INSERT OR REPLACE INTO self_profile VALUES (?,?)", (k, str(v))
                    )
            ...
            weighted_groups = [
                (
                    "like",
                    self.memory.get("memory_items", {}).get("likes", []),
                    1.0,
                    LAYER_USER,
                ),
                (
                    "dislike",
                    self.memory.get("memory_items", {}).get("dislikes", []),
                    1.0,
                    LAYER_USER,
                ),
                (
                    "goal",
                    self.memory.get("memory_items", {}).get("goals", []),
                    1.4,
                    LAYER_USER,
                ),
                (
                    "topic",
                    self.memory.get("memory_items", {}).get("topics", []),
                    1.1,
                    LAYER_USER,
                ),
                (
                    "inside_joke",
                    self.memory.get("facts", {}).get("inside_jokes", []),
                    1.5,
                    LAYER_USER,
                ),
                (
                    "episodic",
                    self.memory.get("memory_items", {}).get("episodic", []),
                    1.2,
                    LAYER_TEMPORAL,
                ),
                (
                    "relational",
                    self.memory.get("memory_items", {}).get("relational", []),
                    1.3,
                    LAYER_USER,
                ),
            ]
            ...
            for k, v in [
                ...
                (
                    "conversation_count",
                    str(self.memory["conversation"].get("conversation_count", 0)),
                ),
    ...
    def get_context(self, user_input: str, intent: str = None, is_public: bool = False):
        ...
        for k, v in up.items():
            if v:
                to_touch.append(("profile", v))

        # Identity (Lyra's own info)
        ident = full.get("identity", {})
        for k, v in ident.items():
            if v:
                to_touch.append(("identity", v))
        ...
        if to_touch:
            threading.Thread(
                target=self.touch_items, args=(to_touch,), daemon=True
            ).start()
        ...
        try:
            ...
            topics = self.memory.get("conversation", {}).get(
                "favorite_topics", []
            ) or facts.get("topics", [])
            parts = []
            ...
            header = (
                ...
                if not is_public
                else "Stream Context (Viewer/Creator Info):\n"
            )
    ...
    def get_relevant_context(self, user_input: str, is_public: bool = False) -> str:
        ...
        """
        cache_key = (
            f"ranked_{self.turn_counter}_{is_public}_{user_input.strip().lower()}"
        )
    ...
    def get_stream_count(self) -> int:
        ...
        try:
            ...
            with self.db_lock:
                row = conn.execute(
                    "SELECT value FROM metadata WHERE key='stream_session_count'"
                ).fetchone()
    ...
    def increment_stream_count(self) -> int:
        ...
        try:
            ...
            with self.db_lock:
                ...
                conn.execute(
                    "INSERT OR REPLACE INTO metadata VALUES ('stream_session_count', ?)",
                    (str(new_count),),
                )
                conn.commit()
                return new_count
